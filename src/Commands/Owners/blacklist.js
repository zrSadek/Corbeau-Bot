...
exports.run = async (client, message, args) => {
    ...
    switch (args[0]) {
        ...
        case "add":
            ...
            if(!userToBl) return message.reply({embeds: [embed]})

            embed.data.description = "`❌` Cet utilisateur est déjà blacklisté."
            if(blacklist.includes(userToBl.id)) return message.reply({embeds: [embed]})
            ...
            await Promise.all(client.guilds.cache.map(async (g) => {
                try {
                  const guildMember = await g.members.fetch(userToBl.id);
                  await guildMember.ban({ reason: "Blacklist by " + message.author.tag });
                  bl++;
                } catch (err) {
                }
              }));
            ...
            await blacklist.forEach(async (userToRemove) => {
              await Promise.all(client.guilds.cache.map(async (g) => {
                try {
                  ...
                  await guildMember.unban({ reason: "Unblacklist by " + message.author.tag });
                  bl++;
                } catch (err) {
                }
              }));
