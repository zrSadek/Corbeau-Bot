...
exports.help = {
    name: 'soutien',
    description: "Te permet d'âttribuer un rôle de soutien.",
    perms: 3,
}
...
exports.run = async (client, message, args) => {
    ...
    async function updateEmbed(i) {        
        ...
        .addFields(
        { name: "🚦 Etat du soutien", value: `${config.state === undefined ? "`🔴`" : config.state ? "`🟢`" : "`🔴`"} - ${config.state === undefined ? "Désactivé" : config.state ? "Activé" : "Désactivé"}` },
        { name: "👤 Rôle à attribuer", value: config.role.replace("<@&undefined>", "Aucun") },
